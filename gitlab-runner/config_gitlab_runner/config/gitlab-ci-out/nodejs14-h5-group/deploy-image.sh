...
do
  ...
  if [ `cat ".gitlab-ci.type"` == 'deploy' ]; then
    if [ -f "${local_cache_path}/${project_path}/${is_need_deploy_image_filename}" ]; then
        echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> deploy image for $project_path <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"
        ...
        cat k8s.yaml
        repo_gen_path=/build/cache/data/k8s_gen/$CI_PROJECT_NAME/"${CI_COMMIT_REF_NAME}-${CI_COMMIT_TAG}"/${project_path}/
        ...
        /bin/rm -rf "${local_cache_path}/${project_path}/${is_need_deploy_image_filename}"
